...
| MAIN Help/menu discoverability | command discoverability parity | AppShell legacy menu blocks + deterministic enabled/disabled action state affordances | Partial (final pass) | EPIC-UX-01 + EPIC-UX-02 | `menu-discoverability.e2e.spec.cjs` |
| Appendix A-7 | calc algorithm parity + evidence lane | `calc.rs`, parity tests, parity manifest/checksum preflight, CI strict-readiness gates, parity truth scripts | Partial (strict-ready) | EPIC-EVIDENCE-01 + EPIC-EVIDENCE-01B | calc parity suites + `parity_fixture_preflight.rs` + `apps/desktop/scripts/parity-status.cjs` + `apps/desktop/scripts/parity-truth.cjs` |
