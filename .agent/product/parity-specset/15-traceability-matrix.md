...
| Ch 8-2/8-3/8-5/8-8/8-9 | marks entry/update/weight | `MarksScreen.tsx`, `grid.*`, `assessments.*`, `entries.*`, `marks.pref.hideDeleted.*` | Implemented (core) | EPIC-CORE-01 | `marks-action-strip.e2e.spec.cjs`, `marks-hide-deleted.e2e.spec.cjs`, `marks-update-all.e2e.spec.cjs`, `entries_delete_semantics.rs`, `entries_clone_roundtrip.rs`, `assessments_hide_deleted_like.rs` |
...
| Ch 6 planner | unit/lesson planner | `PlannerScreen.tsx`, `planner.units.*`, `planner.lessons.*`, `planner.publish.*` | Implemented (core depth) | EPIC-PLANNER-01 + EPIC-PLANNER-03 | `planner-units.e2e.spec.cjs`, `planner-lessons.e2e.spec.cjs`, `planner-publish.e2e.spec.cjs`, `planner_units_clone.rs`, `planner_lessons_copy_forward.rs`, `planner-clone-copyforward.e2e.spec.cjs` |
| Ch 6-8/6-11 | course description/time management | `CourseDescriptionScreen.tsx`, `courseDescription.*`, planner report templates | Implemented (core depth) | EPIC-PLANNER-02 + EPIC-PLANNER-03 | `course-description.e2e.spec.cjs`, `planner-report-export.e2e.spec.cjs`, `course_description_options.rs`, `course-description-options.e2e.spec.cjs` |
| MAIN Setup menu | setup subdomains | `SetupAdminScreen.tsx` + `CalcSettingsScreen.tsx` + `setup.get/update` (marks/exchange/analytics + attendance/comments/reports/security/printer defaults) | Partial (depth-expanded) | EPIC-SETUP-01 + EPIC-SETUP-02 + EPIC-SETUP-03 | `setup-admin.e2e.spec.cjs`, `setup-admin-depth.e2e.spec.cjs`, `setup_admin_ipc.rs`, `setup_defaults_extended.rs`, `setup_sections_depth.rs` |
...
| MAIN Help/menu discoverability | command discoverability parity | AppShell legacy action registry + generated menu groups + Legacy Actions Map screen + deterministic enabled/disabled action state affordances | Implemented (functional) | EPIC-UX-01 + EPIC-UX-02 | `menu-discoverability.e2e.spec.cjs`, `legacy-actions-map.e2e.spec.cjs`, `screen-action-label-consistency.e2e.spec.cjs` |
| Appendix A-7 | calc algorithm parity + evidence lane | `calc.rs`, parity tests, parity manifest/checksum preflight, CI strict-readiness gates, parity truth scripts, strict-intake checklist output | Partial (strict-ready) | EPIC-EVIDENCE-01 + EPIC-EVIDENCE-01B + EPIC-EVIDENCE-02-PREP | calc parity suites + `parity_fixture_preflight.rs` + `apps/desktop/scripts/parity-status.cjs` + `apps/desktop/scripts/__tests__/parity-status.test.cjs` + `apps/desktop/scripts/parity-truth.cjs` |
