...
test("dashboard re-import attached source updates class and preserves membership overrides", async () => {
  ...
  try {
    ...
    await expect(page.getByTestId("class-attach-legacy-btn")).toBeVisible();
    await expect(page.getByTestId("class-reimport-attached-btn")).toBeVisible();
    await expect(page.getByTestId("class-attached-source-panel")).toBeVisible();

    const legacyFolder = `${repoRoot}/fixtures/legacy/Sample25/MB8D25`;
    await page.evaluate(async ({ classId, legacyFolder }) => {
      await window.markbook.request("classes.importLink.set", {
        classId,
        legacyClassFolderPath: legacyFolder
      });
      await window.markbook.request("classes.updateFromAttachedLegacy", {
        classId,
    ...
    await expect.poll(async () => {
      ...
      return Boolean(meta?.meta?.lastImportedAt);
    }).toBeTruthy();
