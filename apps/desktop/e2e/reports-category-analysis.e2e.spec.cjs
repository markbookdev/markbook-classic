...
test("export category analysis PDF writes a non-empty file", async () => {
  ...
  try {
    ...
    const outDir = mkdtemp("markbook-pdf-cat-");
    const outPath = path.join(outDir, "category-analysis.pdf");

    await page.evaluate(async ({ classId, markSetId, outPath }) => {
      if (!window.__markbookTest?.exportCategoryAnalysisPdfToPath) {
        throw new Error("missing window.__markbookTest.exportCategoryAnalysisPdfToPath");
      }
